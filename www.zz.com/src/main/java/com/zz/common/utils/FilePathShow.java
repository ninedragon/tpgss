...
public class FilePathShow {
	...
	public static void find(String pathName, int depth) throws IOException {
		...
		// 获取此目录下的所有文件名与目录名
		String[] fileList = dirFile.list();
		for (int j = 0; j < dirFile.listFiles().length; j++) {
			dirRoot.add(dirFile.listFiles()[j].getAbsolutePath());
		}
		int currentDepth = depth + 1;
		for (int i = 0; i < fileList.length; i++) {
			// 遍历文件目录
...
//					dirRoot.add(file.getAbsolutePath());
				} else {
					for (int j = 0; j < file.listFiles().length; j++) {
						dirRoot.add(file.listFiles()[j].getAbsolutePath());
					}
				}
			} else {
				// 如果是文件，则直接输出文件名
...
public static void main(String[] args) throws IOException {
	    find("E:",2);
	    List<String> dirRoot2 = FilePathShow.getDirRoot();
	    System.out.println(dirRoot2);
}
}
