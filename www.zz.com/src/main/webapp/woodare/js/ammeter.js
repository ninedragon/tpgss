	...
	if ((null != rowId && "" != rowId) && (null != tableBoxId && "" != tableBoxId)) {
		...
		if (null != data && data.length > 0) {
			...
			if (null != rootList && rootList.length > 0) { //有表箱集合的
				...
				for (var i = 0; i < rootList.length; i++) {
					...
					if (null != ammeterList && ammeterList.length > 0) { //有电表集合的
						...
						for (var x = 0; x < ammeterList.length; x++) {
							...
							//电表文本引入
							createMeterText(layerSnap, "meterbox_" + x_rowId, (ammeterX - 10), (ammeterY - 25), "M0005", x_name);
							//table
				...
				createMeterText(layerSnap, "meterBoxText_" + i_epuParentId, (tableBoTxtX - 100), (tableBoTxtY - 120), "M0004", i_epuName);
				//table
				//展示table信息
				javaScriptObj.meterBoxXYJSON = {
					KEY : i_epuParentId,
					X : (tempBranchBoxX + 30),
					Y : cabinetsY + 135
				};
		...
		success : function(allData) {
			...
			if (bool) {
				...
				for (var i = 0; i < faultNowMeterBoxArray.length; i++) {
					...
					meterTxtError("meterBoxText_" + key, epuName, "故障原因:" + faultTypeName, "M0004", "error");
				...
				for (var i = 0; i < faultNowMeterArray.length; i++) {
					...
					meterTxtError("meterbox_" + key, epuName, "故障原因:" + faultTypeName, "M0005", "error");
...
function clearFalut(faultNowMeterArray, faultNowMeterBoxArray) {
	for (var i = 0; i < faultNowMeterBoxArray.length; i++) {
		...
		meterTxtError("meterBoxText_" + key, epuName, "", "M0004", "fText");
	...
	for (var i = 0; i < faultNowMeterArray.length; i++) {
		...
		meterTxtError("meterbox_" + key, epuName, "", "M0005", "fText");
...
function createMeterText(layer, id, x, y, type, name) {
	...
	if (name && name.length) {
		var splitCount = 21;
		var size = 6;
		name = subStringName(name, type);//根据类型限制显示文字长度
 ...
 * 设置文字渲染样式
 * **/
function meterTxtError(key, epuName, epuNameError, type, cls) {
	var x = $("#" + key).attr("x");
	var y = $("#" + key).attr("y");
	var k = 0;
	x = parseInt(x);
	...
	if (epuName && epuName.length) {
		var size = 6;
		javaScriptObj.svgObj.select("g[id='" + key + "']").each(function() {
			$(this).children("text").each(function() {
				$(this).remove();
			});
			$(this).children("title").remove(); //清除原始标题
		});
		var g = javaScriptObj.svgObj.select("g[id='" + key + "']");
		g.append('title').text(epuName);
		epuName = subStringName(epuName, type);//根据类型限制显示文字长度
 ...
 * 根据类型限制显示文字长度
 * */
function subStringName(epuName, type) {
	if ("" != type && null != type) {
		var splitCount = 21;
		if (type == "M0004") {
			splitCount = 32;
