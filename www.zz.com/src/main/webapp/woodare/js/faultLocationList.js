...
function initList(pageNo) {
	$("#loadingDiv").show();
	$(".pagination").html("");//清空分页区域
	var basePath = getRootPath_web();
	var cation = $.trim($("#action").val() || "");
	var fault_type = $.trim($("#fault_secondLevel_type").val()) || $.trim($("#fault_type").val()); //优先拿二级故障原因，反之一级
	$.ajax({
...
function showFaultBase(fault_base_id, cation) {
	$("#loadingDiv").show();
	$.post(getRootPath_web() + '/fault/ajax_faultDetails', {
 ...
 * 二级故障原因展示
 **/
function changeFault(obj) {
	var text = $(obj).find("option:selected").text();
	var edit_type = $(obj).val();

	var htmls = "";
	$("#secondLevelFalut").html(htmls);
	if ("" == edit_type || null == edit_type) {
		return false;
	}
	var basePath = getRootPath_web();
	var codeTypes = "F0001,F0002,F0003,F0004"; //默认所有
	...
	$.ajax({
		...
		success : function(data) {
			if (data) {
				if (null != data && data.length > 0) {
					var length = data.length;
					htmls += "<span>" + text + " </span>"; //fault_secondLevel_type
					htmls += " <select id=\"fault_secondLevel_type\" name=\"fault_secondLevel_type\">";
					var key ="";
					for (var i = 0; i < length; i++) {
						key += data[i].code + ",";
					}
					key = key.substring(0,key.length - 1 );
					htmls += '<option value="'+key+'">--请选择--</option>';
					for (var i = 0; i < length; i++) {
						htmls += '<option value="' + data[i].code + '">' + data[i].code_NAME + '</option>';
					}
					htmls += '</select>';
					$("#secondLevelFalut").html(htmls);
				}
...
function clearQueryParam() {
	...
	$("#fault_type").val(""); //清空
	$("#is_repaired").val(""); //清空
	$("#secondLevelFalut").html(""); //清空
	$("#str_occur_time").html(""); //清空
}