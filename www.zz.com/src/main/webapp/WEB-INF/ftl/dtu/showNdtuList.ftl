		...
		var epAdd= {};
			 ...
			 epAdd.submitFun=function submitFun(flag){
			  ...
			  epAdd.saveShow=function saveShow(){
				...
				$("#saveDiv").show();
				$("#saveDiv #epuTitle").html("新增NDTU设备");
		          ...
		          epuEdit.saveShow=function saveShow(id){
						...
						$.ajax({
							...
							success : function(data) {							
								  ...
								  $.ajax({
										url : "${basePath}/epu/queryNdutUseDistrictIdAndAddressId.shtml",
										type : 'POST',
										dataType : 'json',
										async : false,
										data : {
											 ...
											 var queryNum = data.queryNum;
												if(queryNum>0)
												{
													...
													$("#saveDiv #cAddressid").attr("disabled",true );
													 ...
													 $("#saveDiv #cChannelnum").attr("disabled",true );	
													 $("#saveDiv #showMsg").html("该NDTU设备已经被使用.区域码,地址号,通道数不能修改！");	
												}
												else												
												{
													$("#saveDiv #cDistrictbcdid").attr("disabled",false );
										       ...
										       $("#saveDiv #cDesp").val(epuInfo.cDesp );						
										        $("#saveDiv").show();
										        $("#saveDiv #epuTitle").html("编辑NDTU设备");
						                       $("#loadingDiv").hide();		
										}        
										});
...
<div class="wapp-layer"  id="saveDiv">
	...
	<h4><div id='epuTitle'>新增NDTU设备</div><span class="close-js"  onclick="$('#saveDiv').hide();">关闭</span></h4>
