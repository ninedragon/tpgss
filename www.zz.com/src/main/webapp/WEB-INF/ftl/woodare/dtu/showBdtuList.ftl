		...
		<script language="javascript" type="text/javascript" src="${basePath}/js/My97DatePicker/WdatePicker.js"></script>
			...
			so.init(function(){
			  ...
			  so.editEpuInfo=function editEpuInfo(id) {				 
				 window.location="${basePath}/bDtu/editInit.shtml?id=" + id;
			 };
			 so.delEpuInfo=function delEpuInfo(id) {				
				if(confirm('确实要删除该内容吗?')){
	   				  $("#loadingDiv").show();
	   				 $.ajax({
	   		  			    url:"${basePath}/bDtu/delDtu.shtml",
	   		  			    type:'POST',
	   		  			    dataType:'json',
	   		  			    data:{id:id},
	   		  			    success:function(data){
	   		  			    	...
	   		  			    	$("#loadingDiv").hide();
	   		  					 ...
	   		  					 window.location="${basePath}/bDtu/showDtuList.shtml";
	   		  			    	$("#loadingDiv").hide();
	   		  			    	}
	   		  			    }
			  ...
			  function initList(pageNo) {
			        ...
			        $.ajax({
			            url: "${basePath}/bDtu/queryDtuList.shtml",
			            type: 'POST',
			            dataType: 'json',
			            async:false,
			            data: {			          
		...
		var epAdd= {};
			 ...
			 epAdd.submitFun=function submitFun(flag){
 				...
 				if (flag=="1")
				{
				   ...
				   $("#loadingDiv").show();
					$.ajax({
						url : "${basePath}/bDtu/updateDtu.shtml",
						type : 'POST',
						dataType : 'json',
						async : false,
						data : {
			...
			//初始化页面数据
			 ...
			 epuEdit.registEvent=function registEvent() {
		          ...
		          epuEdit.saveShow=function saveShow(id){
							...
							$("#loadingDiv").show();		 
						$.ajax({
							url : "${basePath}/bDtu/editInit.shtml",
							type : 'POST',
							dataType : 'json',
							async : false,
							data : {
	...
	<h4>新增bdtu设备<span class="close-js"  onclick="$('#saveDiv').hide();">关闭</span></h4>
	  <input name="id" type="hidden" id="id"  value="">
        ...
        <div class="edit" style="height:auto">
             <lable>
                <span>区域号</span>
                  <input name="cDistrictbcdid" id="cDistrictbcdid"   type="text" class="text request" title="区域号" maxlength="100"/>
            </lable>
             <lable>
                <span>地址号</span>
