...
<html lang="zh-cn">
	<head>
		...
		<script >
			...
			//初始化页面数据
			 function initPage() {
				  var epuProvince="${epuInfo.epuProvince}";
				    var epuCity="${epuInfo.epuCity}";
					 var epuDistrict="${epuInfo.epuDistrict}";
					 var epuType="${epuInfo.epuType}";
	                    var parentEpuType="${epuInfo.epuParentType}";
	                    var epuParentId="${epuInfo.epuParentId}";
	                    var districtId="${epuInfo.districtId}";
	                    var addressId="${epuInfo.addressId}";
	                    var channelId="${epuInfo.channelId}";
	                    var lineId="${epuInfo.lineId}";
	                    if(epuType=='M0001' )
				        {
		            ...
		            $.ajax({
		                ...
		                success: function (a) {
		                    ...
		                    for (var i = 0; i < length; i++) {
		                    	if(epuProvince==a[i].provinceId)
		                    		{
		                    		   $('#epuProvince').append('<option value="' + a[i].provinceId + '" selected="selected">' + a[i].provinceNameCn + '</option>');
		                    		}else
		         ...
		         // 城市
		            $.ajax({
		                ...
		                success: function (a) {
		                    ...
		                    for (var i = 0; i < length; i++) {
		                    	if(epuCity==a[i].cityCode )
		                    	{
		                    		  $('#epuCity').append('<option value="' + a[i].cityCode + '" selected="selected">' + a[i].cityNameCn + '</option>');
		                    	}
		                    	else
		                    	{
		         ...
		         // 区域
		        	$.ajax({
						...
						success : function(a) {
							...
							for (var i = 0; i < length; i++) {
								if(epuDistrict==a[i].cityCode)
		                    	{
									$('#epuDistrict').append('<option value="' + a[i].cityCode+ '" selected="selected">' + a[i].cityNameCn+ '</option>');	
		                    	}
								else
								{
									$('#epuDistrict').append('<option value="' + a[i].cityCode+ '">' + a[i].cityNameCn+ '</option>');	
								}
		         ...
		         //设备类型和上级设备类型
		           $.ajax({
		                ...
		                success: function (a) {
		                    ...
		                    for (var i = 0; i < length; i++) {
		                    	if(epuType==a.epuTypeBean[i].code)
		                    	{
		                    		  $('#epuType').append('<option value="' + a.epuTypeBean[i].code + '" selected="selected">' + a.epuTypeBean[i].codeName + '</option>');
		                    	}
		                    	else
		                    	{
		                    	...
		                    	if(parentEpuType==a.epuTypeBean[i].code)
	                    		{
		                    		$('#parentEpuType').append('<option value="' + a.epuTypeBean[i].code + '" selected="selected">' + a.epuTypeBean[i].codeName + '</option>');
	                    		}
	                    	   else
	                    	   {
		          ...
		          $.ajax({ type: "post",
				   			                ...
				   			                success: function (a) {
				   			                    ...
				   			                    for (var i = 0; i < epuParentList.length; i++) {
				   			                    	if(epuParentId==epuParentList[i].rowId)
				   			                    	{
				   			                    	  $('#epuParentId').append('<option value="' + epuParentList[i].rowId + '" selected="selected">' + epuParentList[i].epuName + '</option>');
				   			                    	}
				   			                    	else
				   			                    	{
		         ...
		         //关联终端bcd编号
			                	   ...
			                	   $.ajax({
			   			                ...
			   			                success: function (a) {
			   			                    ...
			   			                    for (var i = 0; i < epuInfoList.length; i++) {
			   			                    	if(districtId==epuInfoList[i].districtId)
			   			                    	{
			   			                    	  $('#districtId').append('<option value="' + epuInfoList[i].districtId + '" selected="selected">' + epuInfoList[i].districtId + '</option>');
			   			                    	}
			   			                    	else
			   			                    	{
			                	   ...
			                	   $.ajax({
						                ...
						                success: function (a) {
						                    ...
						                    for (var i = 0; i < epuInfoList.length; i++) {
						                    	if(addressId==epuInfoList[i].addressId)
						                    	{
						                    		 $('#addressId').append('<option value="' + epuInfoList[i].addressId + '" selected="selected">' + epuInfoList[i].addressId + '</option>');
						                    	}
						                    	else
			                	   ...
			                	   $.ajax({
			   			                ...
			   			                success: function (a) {
			   			                    ...
			   			                    for (var i = 0; i < epuInfoList.length; i++) {
			   			                    	if(channelId==epuInfoList[i].channelId)
						                    	{
						                    		 $('#channelId').append('<option value="' + epuInfoList[i].channelId + '" selected="selected">' + epuInfoList[i].channelId + '</option>');
						                    	}
						                    	else
		...
		<div class="container" style="padding-bottom: 15px;min-height: 300px; margin-top: 40px;">
			<div class="row">
				...
				<div class="col-md-10">
		           ...
		           <input name="rowId" type="hidden" id="rowId"  value="${epuInfo.rowId}">
				<table border="0" cellspacing="0" cellpadding="0" class="edit-wapper">					
					...
					<tr>
						...
						<td >
							<div class="mode"><input name="epuName"  id="epuName" value="${epuInfo.epuName}" type="text" class="text request" title="设备名称" maxlength="100"></div>
						</td>
						   <td >
						...
						<td >
						<div class="mode"><input name="epuLocal"  id="epuLocal" type="text" class="text request" title="设备位置" maxlength="100" value="${epuInfo.epuLocal}">&nbsp;&nbsp;
					...
					<tr id="hiddenDev1">
						...
						<div class="mode"><input name="lineId" id="lineId"  value="${epuInfo.lineId}" type="text" class="text request" title="线缆号" maxlength="100"></div>
